...
export const useApprove = (tokenAddress?:string, spenderAddress?:string) => {
  ...
  const handleApprove = useCallback(async () => {
    const txHash = await approve(tokenAddress, spenderAddress, ethereum, account)
    return txHash
  }, [account, ethereum, tokenAddress, spenderAddress])

  const fetchAllowance = useCallback(async () => {
    const allowance = await getAllowance(tokenAddress, spenderAddress, ethereum, account);
    if (allowance) {
